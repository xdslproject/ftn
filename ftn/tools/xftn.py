...
class xftnMain:
    ...
    def set_parser_arguments(self, parser):
        ...
        parser.add_argument(
            "-omp", "--openmp", action="store_true", help="Enable OpenMP"
        )
        parser.add_argument(
            "-fomp", "--fopenmp", action="store_true", help="Enable OpenMP"
        )
        parser.add_argument(
            "-d",
            "--debug",
        ...
        parser.add_argument(
            "--stages",
            default=None,
            help="Specify which stages will run (a combination of: flang, pre, ftn, post, "
            "mlir, obj, clang) in comma separated list without spaces",
        )
        parser.add_argument(
            "-v",
            "--verbose",
    ...
    def get_output_type_from_out_name(self, output_name):
        if "." not in output_name:
            return xftnMain.OutputType.EXECUTABLE
        elif output_name.endswith(".o"):
            return xftnMain.OutputType.OBJECT
        elif output_name.endswith(".bc"):
            return xftnMain.OutputType.BITCODE
        elif output_name.endswith(".mlir"):
            return xftnMain.OutputType.MLIR

    def enable_disable_stages_by_output_type(self, options_db, output_type):
        ...
        options_db["run_postprocess_stage"] = (
            output_type == xftnMain.OutputType.EXECUTABLE
            or output_type == xftnMain.OutputType.OBJECT
            or output_type == xftnMain.OutputType.BITCODE
        )
        options_db["run_mlir_to_llvmir_stage"] = options_db["run_postprocess_stage"]
        options_db["run_create_object_stage"] = (
            output_type == xftnMain.OutputType.OBJECT
        )
        options_db["run_build_executable_stage"] = (
            output_type == xftnMain.OutputType.EXECUTABLE
        )
    ...
    def build_options_db_from_args(self, args):
        ...
        self.validate_source_filename(src_fn)
        ...
        output_type = self.get_output_type_from_out_name(options_db["out"])
        options_db["output_type"] = output_type
        self.enable_disable_stages_by_output_type(options_db, output_type)
    ...
    def validate_source_filename(self, src_fn):
    ...
    def post_stage_check(self, check_fn, verbose_level, executable=False):
