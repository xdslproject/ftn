...
class RewriteTarget(RewritePattern):
    ...
    def match_and_rewrite(self, op: device.KernelCreate, rewriter: PatternRewriter, /):
        ...
        for var in op.mapped_data:
            ...
            var_type = builtin.MemRefType(
                var.type.element_type, var.type.shape, var.type.layout
            )
        ...
        dev_func = func.FuncOp.from_region(
            "device_kernel", arg_types, [], dev_func_body
        )
        ...
        for block_arg, arg_type in zip(dev_func_body.block.args, arg_types):
            new_block_arg = dev_func_body.block.insert_arg(
                arg_type, len(dev_func_body.block.args)
            )
        ...
        assert (
            dev_func_body.block.last_op is not None
        ), "The last operation in the device function block must not be None"
...
class ExtractTarget(ModulePass):
    ...
    def apply(self, ctx: Context, module: builtin.ModuleOp):
        assert "omp.target_triples" in module.attributes

        assert len(module.attributes["omp.target_triples"]) == 1
        target = module.attributes["omp.target_triples"].data[0]
        ...
        new_module = builtin.ModuleOp(rw_target.target_ops, {"target": target})
